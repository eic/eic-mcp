...
Docker, which publishes no ports — either run your client inside eic-shell, or
start eic-shell with the ports published (`eic-mcp docker-args` prints the
`-p` flags for the enabled servers; inside Docker the servers bind all
container interfaces, and the configs advertise `127.0.0.1`):

```bash
DOCKER_OPTIONS="$(./eic-shell -- eic-mcp docker-args)" ./eic-shell
```

`DOCKER_OPTIONS` needs an eic-shell launcher generated by the current
installer (`grep -q DOCKER_OPTIONS eic-shell || re-run install.sh`).
